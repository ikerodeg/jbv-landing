...
1. Añadir `engines`, `private: true`, `description` y `license` en `package.json`
2. Mover `astro` de `dependencies` a `devDependencies`
3. Añadir scripts `lint`, `format` y `check` en `package.json`
4. Crear `.nvmrc` con la versión de Node requerida
5. Unificar `site` en `astro.config.mjs` con el canonical real (`jbv-gruas.es`)
6. Instalar `@astrojs/sitemap` y configurar en `astro.config.mjs`
7. Añadir path aliases (`@components`, `@layouts`, `@data`, `@styles`) en `tsconfig.json`
8. Crear `public/robots.txt` con referencia al sitemap
9. Crear directorio `src/data/` y extraer datos compartidos (`navigation.ts`, `services.ts`, `steps.ts`, `reasons.ts`)
10. Crear `src/components/ui/Icon.astro` para centralizar SVGs reutilizados (WhatsApp, teléfono)
...
33. Eliminar atributo no estándar `writingsuggestions="false"` del textarea
